...
import {getTipoAmenazas, deleteTipoAmenaza, createTipoAmenaza, updateTipoAmenaza} from '../../services/amenazas/tipoamenaza/tipoAmenazasData';
...
const TablaTAm = () => {
  ...
  const handleGuardar = async () => {
    if (!formulario.Descripcion || formulario.Descripcion.trim() === "") {
     ...
     const dataToSend = {
      descripcion: formulario.Descripcion, // Mapea el campo correctamente
    };
    if (modificarIndex !== -1) {
      // Modificar el dato existente en la posición 'modificarIndex'
      await updateTipoAmenaza(modificarIndex, dataToSend);
      // Now update the 'datos' state with the updated data
      const tipoAmenazaData = await getTipoAmenazas();
      setDatos(tipoAmenazaData);
      toast.success("Tipo de amenaza modificado correctamente");
    } else {
      try {
        await createTipoAmenaza(dataToSend);
      }
      catch (error) { console.log(error) };
      // Now fetch the updated data and update the 'datos' state
      const tipoAmenazaData = await getTipoAmenazas();
      setDatos(tipoAmenazaData);
      toast.success("Tipo de amenaza creado correctamente");
    }
  ...
  const handleConfirmDelete = async () => {
    const codigoToDelete = datos[deleteIndex].tia_codigo;
    await deleteTipoAmenaza(codigoToDelete);
    toast.success("Tipo de amenaza eliminado correctamente");
    const tiposAmenazaData = await getTipoAmenazas();
    setDatos(tiposAmenazaData);
  ...
  const datosFiltrados = datos.filter((dato) =>
    dato.tia_descripcion && dato.tia_descripcion.toLowerCase().includes(busqueda.toLowerCase())
  );
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)}>
        ...
        >
          ...
          <Box>
            ...
            <Button variant="contained" color="primary" onClick={handleGuardar}   disabled={!formulario.Descripcion || formulario.Descripcion.trim() === ""}>
              Guardar
            </Button>
          </Box>
        </Box>
      ...
      <Modal open={confirmOpen} onClose={handleCancelDelete}>
        ...
        >
          <Typography variant="h6" gutterBottom>
            ¿Está seguro que desea eliminar este tipo de amenaza?
          </Typography>
